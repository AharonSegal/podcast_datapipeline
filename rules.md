...
- the entire response must be a single markdown file
- no explanatory text outside file contents when generating a scaffold
- only file contents are allowed in the output
- each file must be clearly labeled by its path
- use clear separators
  - service separator: a full line of `=` characters
  - file separator: a full line of `-` characters
- at the end of the markdown file include one bash command that
  - creates the entire project directory structure
  - creates all empty files that are part of the project
  - must be a single command that can be copy pasted
